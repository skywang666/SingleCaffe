...
LOG=/HOME/nsccgz_pcheng_1/skywang/caffe/caffe-skywang/examples/mnist/lenet_rmsprop/rmsprop-${tag}-$N-$n-$t-$c.log
...
yhrun -N $N -n $n --ntasks-per-node=1 -c $c $TOOLS/caffe train \
  --solver=/HOME/nsccgz_pcheng_1/skywang/caffe/caffe-skywang/examples/mnist/lenet_solver_rmsprop.prototxt  \
  --thread=$t\
  &>>$LOG
