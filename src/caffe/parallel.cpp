...
#ifdef USE_OMPI
template <typename Dtype> class MpiDataType;
template<> class MpiDataType<float> {
 public:
  static const MPI_Datatype type = MPI_FLOAT;
};
template<> class MpiDataType<double> {
 public:
  static const MPI_Datatype type = MPI_DOUBLE;
};

template<typename Dtype>
CPUParams<Dtype>::CPUParams(shared_ptr<Solver<Dtype> > solver)
  : Params<Dtype>(solver) {
  // Allocate device buffers
  data_ = (Dtype*)malloc(size_ * sizeof(Dtype));
  diff_ = (Dtype*)malloc(size_ * sizeof(Dtype));
  caffe_set(size_, Dtype(0), data_);
...
template<typename Dtype>
OMPICOM<Dtype>::OMPICOM(shared_ptr<Solver<Dtype> > solver)
  : CPUParams<Dtype>(solver), solver_(solver) {
...
template<typename Dtype>
void OMPICOM<Dtype>::syncData() {
  caffe_copy(size_, position[0], data_);
}

template<typename Dtype>
void OMPICOM<Dtype>::syncNodeData() {
  if(Caffe::mpi_count() < 2)return;
  MPI_Barrier(MPI_COMM_WORLD);

  if(Caffe::mpi_rank() == 0 && Caffe::process_root_thread())
    for(int i = 1; i < Caffe::mpi_count(); i++)
      MPI_Send(data_, size_, MpiDataType<Dtype>::type, i, 0, MPI_COMM_WORLD);
  if(Caffe::mpi_rank() > 0 && Caffe::process_root_thread())
    MPI_Recv(data_, size_, MpiDataType<Dtype>::type, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
}

template<typename Dtype>
void OMPICOM<Dtype>::avgDiff(int offset, int size) {
#ifdef USE_DEBUG
#pragma omp critical
  std::cout<<"mpi rank "<<Caffe::mpi_rank()<<" thread rank "<<Caffe::thread_rank()\
    <<" solver rank "<<Caffe::solver_rank()<<" thread count "<<Caffe::thread_count()\
    <<" run avgDiff!"<<std::endl;
#endif
...
#pragma omp critical
  std::cout<<"mpi rank "<<Caffe::mpi_rank()<<" thread rank "<<Caffe::thread_rank()\
    <<" solver rank "<<Caffe::solver_rank()<<" thread count "<<Caffe::thread_count()\
    <<" Diff begin "<<position_diff[rank][1000]<<" "<<position_diff[rank][9999]<<std::endl;
#endif

  //Sum diffs in process
    int step = size/count;
    if(step*count < size) step++;
    Dtype *src = position_diff[0] + offset + rank * step;
  if(count > 1){
    int len = size - rank * step;
    len = len >= step ? step : len;
    for(int i = 1; i < count; i++){
      Dtype *target = position_diff[i] + offset + rank * step;
      caffe_add(len, target, src, src);
    }
  }

//Sum diffs between process
#pragma omp barrier
  if(Caffe::process_root_thread() && Caffe::mpi_rank() > 0 && Caffe::mpi_count() > 1){
    MPI_Send(src, size, MpiDataType<Dtype>::type, 0, 5, MPI_COMM_WORLD);
    MPI_Recv(src, size, MpiDataType<Dtype>::type, 0, 6, MPI_COMM_WORLD, MPI_STATUS_IGNORE);   
  }

  if(Caffe::process_root_thread() && Caffe::mpi_rank() == 0 && Caffe::mpi_count() > 1){
    Dtype *buffer;
    if(count == 1)
      buffer = (Dtype*)malloc(size * sizeof(Dtype)); 
    else
      buffer = position_diff[1];
    for(int i = 1; i < Caffe::mpi_count(); i++){
      MPI_Recv(buffer, size, MpiDataType<Dtype>::type, i, 5, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      caffe_add(size, buffer, src, src);
    }
    if(count == 1)
      free(buffer);
    for(int j = 1; j < Caffe::mpi_count(); j++)
      MPI_Send(src, size, MpiDataType<Dtype>::type, j, 6, MPI_COMM_WORLD);
  }

//Gradient scaling and commnucation between threads
#pragma omp barrier
  if(Caffe::process_root_thread())
    caffe_scal(size, (Dtype) 1.0 / Caffe::solver_count(), src);
  
#pragma omp barrier
  if(rank > 0){
    Dtype *to = position_diff[rank] + offset;
    Dtype *from = position_diff[0] + offset;    
    caffe_copy(size, from, to);
  }
#pragma omp barrier
#ifdef USE_DEBUG
#pragma omp critical
  std::cout<<"mpi rank "<<Caffe::mpi_rank()<<" thread rank "<<Caffe::thread_rank()\
    <<" solver rank "<<Caffe::solver_rank()<<" thread count "<<Caffe::thread_count()\
    <<" Diff End "<<position_diff[rank][1000]<<" "<<position_diff[rank][9999]<<std::endl;
#endif
#pragma omp barrier
}

template<typename Dtype>
void OMPICOM<Dtype>::on_test(Dtype &loss, vector<Dtype> &score) {
#ifdef USE_DEBUG
#pragma omp critical
  std::cout<<"mpi rank "<<Caffe::mpi_rank()<<" thread rank "<<Caffe::thread_rank()\
    <<" solver rank "<<Caffe::solver_rank()<<" thread count "<<Caffe::thread_count()\
    <<" run On_test!"<<std::endl;
#endif
  int mpi_rank = Caffe::mpi_rank();
  int count = Caffe::thread_count();
  int omp_rank = Caffe::thread_rank();
  int omp_size = Caffe::thread_count();

  loss_[omp_rank] = loss;  // Synchronize loss between threads in a process
  if(omp_rank == 0)
    for(int i = 1; i < omp_size; i++)
      loss_[0] = loss_[0] + loss_[i];
  loss = loss_[0];

  Dtype test_score[score.size()];  //Synchronize score between threads in a process
  for(int i = 0; i < score.size(); i++)
    test_score[i] = score[i];
  ((*this->score_))[omp_rank] = test_score;
#pragma omp barrier
  Dtype result_com[score.size()];
  for(int i = 0; i < score.size(); i++){
    Dtype tmp = 0;
    for(int j = 0; j < omp_size; j++)
      tmp = tmp + ((*this->score_))[j][i];
    result_com[i] = tmp;
  }

#pragma omp barrier
  if(omp_rank == 0 && Caffe::mpi_count() > 1 && mpi_rank == 0){  //Send and recv data between processes
    for(int i = 1; i < Caffe::mpi_count(); i++){
      Dtype loss_buffer;
      Dtype score_buffer[score.size()];
      MPI_Recv(&loss_buffer, 1, MpiDataType<Dtype>::type, i, 3, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      MPI_Recv(score_buffer, score.size(), MpiDataType<Dtype>::type, i, 4, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      loss = loss + loss_buffer;
      for(int j = 0; j < score.size(); j++)
        result_com[j] = result_com[j] + score_buffer[j];
    }
  }

  if(mpi_rank > 0 && Caffe::mpi_count() > 1 && omp_rank == 0){
    MPI_Send(&loss, 1, MpiDataType<Dtype>::type, 0, 3, MPI_COMM_WORLD);
    MPI_Send(result_com, score.size(), MpiDataType<Dtype>::type, 0, 4, MPI_COMM_WORLD);
  }
  ...
  if(mpi_rank == 0 && omp_rank == 0){
    ...
    for(int i = 0; i < score_size; i++)
      score.push_back(result_com[i]);
  }
}

template<typename Dtype>
void OMPICOM<Dtype>::on_gradients_ready() {
#ifdef USE_DEBUG
#pragma omp critical
  std::cout<<"mpi rank "<<Caffe::mpi_rank()<<" thread rank "<<Caffe::thread_rank()\
    <<" solver rank "<<Caffe::solver_rank()<<" thread count "<<Caffe::thread_count()\
    <<" run on_gradients_ready!"<<std::endl;
#endif
...
template<typename Dtype>
void OMPICOM<Dtype>::Run() {
#ifdef USE_DEBUG
  std::cout<<"mpi rank "<<Caffe::mpi_rank()<<" omp ramk "<<Caffe::thread_rank()<<\
      " solver rank "<<Caffe::solver_rank()<<" Run run!"<<std::endl;
#endif
  Caffe::OMPI ompi = Caffe::ompi();
  int thread_count = ompi.thread_count;
  int mpi_rank = Caffe::mpi_rank();
  shared_ptr<Solver<Dtype> > rank0 = solver_;
  vector<Dtype *> postions(thread_count);
  vector<Dtype *> postions_diff(thread_count);
  Dtype loss[thread_count];
  vector<Dtype *> *score = new vector<Dtype *>(thread_count);
  if(thread_count > 1)
   int mutil_omp = 1;

#ifdef USE_DEBUG
  std::cout<<"mpi rank "<<Caffe::mpi_rank()<<" omp ramk "<<Caffe::thread_rank()<<\
      " solver rank "<<Caffe::solver_rank()<<" Run run1!"<<std::endl;
#endif
...
#pragma omp parallel num_threads(thread_count) 
{
  ...
  Caffe::set_solver_count(Caffe::mpi_count()*thread_count);
  Caffe::set_solver_rank(mpi_rank*thread_count + thread_rank);
...
#pragma omp barrier
  ...
  postions[thread_rank] = local->data_;
  postions_diff[thread_rank] = local->diff_;
#pragma omp barrier
  local->position =  postions;
  local->position_diff =  postions_diff;
  local->loss_ = loss;
  local->score_ = score;
...
#pragma omp barrier
  solve_timer.Stop();
  LOG_IF(INFO, Caffe::root_solver()) << "Solve Time: " << solve_timer.MilliSeconds()/1000 << " s.";
  if(thread_rank)
    delete local;
}
//delete []score;
}
INSTANTIATE_CLASS(OMPICOM);
#endif
